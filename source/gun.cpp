...
Magazine::Magazine(int bulletsInitial, int bulletsCapacity) :
    bullets(bulletsInitial),
    capacity(bulletsCapacity) {}

Magazine::Magazine(int bulletsCapacity) :
...
// loops and fires the gun multiple times.
void Gun::Fire(int qty)
{
    for (int i = 0; i < qty; i++)
    {
        Fire(); // This function calls the other fire function multiple times.
    }
}
